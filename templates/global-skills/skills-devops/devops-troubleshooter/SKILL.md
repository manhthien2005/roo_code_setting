...
description: "Incident response, advanced debugging, and modern observability for DevOps. Use for production issues and system reliability."
risk: moderate
source: self
tags: [devops, incident-response, observability, debugging, sre]
---

## Use this skill when

- Debugging production incidents or outages
- Analyzing logs, metrics, or distributed traces
- Troubleshooting container/Kubernetes/cloud issues
- Investigating performance bottlenecks or resource issues

## Do not use this skill when

- Task is pure application code debugging (use `systematic-debugging` skill instead)
- Task is security-focused code review (use `cc-skill-security-review` skill instead)
...
2. Use `execute_command` to run diagnostic commands (kubectl, docker, aws cli, etc.).
3. Use `read_file` to inspect configuration files, logs, and manifests.
4. Use `search_files` to find patterns in config or log files.
5. Apply relevant best practices and validate outcomes.
6. Provide actionable steps and verification.
...
2. **Gather comprehensive data** — use `execute_command` for live diagnostics, `read_file` for configs/logs
